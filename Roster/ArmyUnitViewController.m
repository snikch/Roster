...
{
    if ([[segue identifier] isEqualToString:@"newUnit"]) {
        NSLog(@"Showing unit name controller with moc %@", self.managedObjectContext);
        UnitNameViewController *vc = (UnitNameViewController *)[segue destinationViewController];
        [vc setManagedObjectContext: self.managedObjectContext];
        [vc setArmy:self.army];
    }else if ([[segue identifier] isEqualToString:@"editUnit"]) {
        NSLog(@"Showing unit name controller with moc %@", self.managedObjectContext);
        UnitEditViewController *vc = (UnitEditViewController *)[segue destinationViewController];
        [vc setManagedObjectContext: self.managedObjectContext];
        NSIndexPath *indexPath = [self.tableView indexPathForSelectedRow];
        Unit *unit = (Unit *)[[self fetchedResultsController] objectAtIndexPath:indexPath];
        NSLog(@"Preparing for unit edit segue with unit: %@", unit);
        [vc setManagedObjectContext:self.managedObjectContext];
        [vc setUnit: unit];
    }
}
...
-(void)configureView{
    ...
    if(_army == nil){
        ...
        [toolbarButtons removeObject:self.addUnitButton];
        ...
        if (![toolbarButtons containsObject:self.addUnitButton]) {
            ...
            NSLog(@"Setting toolbar buttons with: %@, %@", self.addUnitButton, toolbarButtons);
...
- (NSFetchedResultsController *)fetchedResultsController
{
    ...
    NSSortDescriptor *sortDescriptor1 = [[NSSortDescriptor alloc] initWithKey:@"classification" ascending:YES];
    ...
    NSFetchedResultsController *aFetchedResultsController = [[NSFetchedResultsController alloc] initWithFetchRequest:fetchRequest managedObjectContext:self.managedObjectContext sectionNameKeyPath:@"classification" cacheName:@"Master"];
